...
exports.createInterface = function(input, output, completer, shell, terminal) {
  ...
  if (arguments.length === 1) {
    ...
    rl = new Interface(input, output, completer, shell, terminal);
  }
  return rl;
};


function Interface(input, output, completer, shell, terminal) {
  if (!(this instanceof Interface)) {
    return new Interface(input, output, completer, shell, terminal);
  }
  ...
  this.shell = shell;
  this.kHistorySize = shell.vm.context.HISTSIZE || 5000;
  ...
  if (!this.terminal) {
    ...
    this.history = [];
    this.historyIndex = -1;
    this.histPath = path.join(shell.vm.context.HOME, shell.vm.context.JSH_HISTORY || '.jsh_history');
...
Interface.prototype.setPrompt = function(prompt, length) {
  ...
  if (length) {
    ...
    this._promptLength = lastLine.replace(/\033\[\d*m/g, '').length;
  }
};
...
Interface.prototype._tabComplete = function() {
  ...
  self.pause();
  self.tabbed++;
  if (self.tabbed >= self.lastCompletions.length) {
    self.tabbed = 0;
  }
  if (self.tabbed >= 0) {
    self.resume();
    if (self.lastCompletions[self.tabbed]) {
...
Interface.prototype._historyNext = function() {
  if (this.historyIndex > 0) {
    ...
    this.historyIndex = -1;
    this.cursor = this.currentWord.length;
    this.line = this.currentWord;
    this._refreshLine();
  }
};
...
Interface.prototype._historyPrev = function() {
  if (this.historyIndex + 1 < this.history.length) {
    if (this.historyIndex === -1) this.currentWord = this.line;
    this.historyIndex++;
    this.line = this.history[this.historyIndex];
    this.cursor = this.line.length; // set cursor to end of line.
...
Interface.prototype._ttyWrite = function(s, key) {
  ...
  if (key.ctrl && key.shift) {
    ...
    switch (key.name) {
      case 'c':
        if (this.line) this.output.write('^C');
        this.output.write('\n');
        this.line = '';
        this.currentWord = '';
        this.cursor = 0;
    ...
    switch (key.name) {
      ...
      case 'up':
        if (lastTabbed >= 0) {
          if (!lastTabbed) this.tabbed = this.lastCompletions.length - 2;
          else this.tabbed = lastTabbed - 2;
          this._tabComplete();
