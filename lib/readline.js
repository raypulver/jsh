...
exports.createInterface = function(input, output, completer, context, terminal) {
  var rl;
  if (arguments.length === 1) {
    rl = new Interface(input);
  } else {
    rl = new Interface(input, output, completer, context, terminal);
  }
  return rl;
};


function Interface(input, output, completer, context, terminal) {
  ...
  if (!this.terminal) {
    ...
    this.history = [];
    this.historyIndex = -1;
    this.histPath = path.join(context.HOME, context.JSH_HISTORY || '.jsh_history');
